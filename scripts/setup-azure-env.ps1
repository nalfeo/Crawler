...
param(
    ...
    # With -IncludeFoundry the script (optionally) provisions a Foundry resource,
    # fetches its endpoint/key, writes a FOUNDRY_* block to .env.local, and (with
    # -SyncGitHubSecrets) syncs the FOUNDRY_* secrets. Deployment aliases come from
    # the shared plan (scripts/azure-foundry-plan.ps1), not from params here.
...
# Shared, pure Foundry catalog helpers. This file has no param block and no
# top-level side effects, so dot-sourcing it does NOT disturb the bound
# parameters above (unlike dot-sourcing setup-azure-resources.ps1 would).
...
$foundryKey = $null
if ($IncludeFoundry) {
    Write-Host "Fetching Azure AI Foundry endpoint and key..." -ForegroundColor Cyan
    $foundryEndpoint = az cognitiveservices account show `
        --name $FoundryAccountName `
        --resource-group $FoundryResourceGroup `
        --subscription $Subscription `
        --query properties.endpoint -o tsv
    if ($LASTEXITCODE -ne 0 -or -not $foundryEndpoint) {
        throw "Failed to retrieve Azure AI Foundry endpoint. Provision it first with -ProvisionResources -IncludeFoundry."
    }
    ...
    if ($LASTEXITCODE -ne 0 -or -not $foundryKey) {
        throw "Failed to retrieve Azure AI Foundry API key."
    }
}
...
"@
        ...
        $foundryBlock = ''
        if ($IncludeFoundry -and $foundryEndpoint -and $foundryKey) {
            $foundryPlan = Get-FoundryDeploymentPlan
            $foundryBlock = Format-FoundryEnvBlock `
                -Endpoint $foundryEndpoint `
                -ApiKey $foundryKey `
                -ApiVersion $FoundryApiVersion `
                -EnvMap $foundryPlan.Env
        }
...
AZURE_OPENAI_API_VERSION=$OpenAIApiVersion$storageBlock$foundryBlock
...
if ($SyncGitHubSecrets) {
    ...
    if ($IncludeFoundry -and $foundryEndpoint -and $foundryKey) {
        Write-Host "Syncing FOUNDRY_* secrets (ADR 0033)..." -ForegroundColor Cyan
        $foundryPlan = Get-FoundryDeploymentPlan
        $foundrySecretValues = @{
            FOUNDRY_ENDPOINT             = $foundryEndpoint
            FOUNDRY_API_KEY              = $foundryKey
            FOUNDRY_API_VERSION          = $FoundryApiVersion
            FOUNDRY_IMAGE_MODEL          = $foundryPlan.Env['FOUNDRY_IMAGE_MODEL']
            FOUNDRY_TEXT_MODEL           = $foundryPlan.Env['FOUNDRY_TEXT_MODEL']
            FOUNDRY_VISION_MODEL         = $foundryPlan.Env['FOUNDRY_VISION_MODEL']
            FOUNDRY_BRIEF_SELECTOR_MODEL = $foundryPlan.Env['FOUNDRY_BRIEF_SELECTOR_MODEL']
        }
        # Drive the loop off the shared name list so the synced keys and the
        # documented/tested contract can never diverge.
        foreach ($name in (Get-FoundrySecretNames)) {
            if (-not $foundrySecretValues.ContainsKey($name)) {
                throw "No value wired for Foundry secret '$name' (Get-FoundrySecretNames drifted from the value map)."
            }
            Set-GitHubSecretValue -Repo $GitHubRepo -Name $name -Value $foundrySecretValues[$name]
        }
    }

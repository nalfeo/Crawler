 ...
 */
export declare function runPaths(root: string, brief: Brief, runId: string): RunPaths;
/** Pure run-id builder. Caller supplies `now` and the prompt so tests are deterministic. */
export declare function makeRunId(now: Date, prompt: string): string;
/** Create the run directory tree. Impure. */
export declare function ensureRunDirs(paths: RunPaths): void;
/** Write the raw multi-variant sheet PNG for a given attempt index. */
export declare function writeSheet(paths: RunPaths, attemptIndex: number, sheet: Buffer): string;
/** Write one variant's raw + processed PNGs and its scorecard. Returns paths. */
export declare function writeVariant(
  paths: RunPaths,
  index: number,
  raw: Buffer,
  processed: Buffer,
...
export declare function writeSummary(paths: RunPaths, summary: RunSummary): string;
/**
 ...
 * Pure.
 */
export declare function rankCandidates(entries: ReadonlyArray<RunSummaryEntry>): RunSummaryEntry[];
/**
 ...
 */
export declare function pickChosen(

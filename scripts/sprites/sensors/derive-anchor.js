 ...
 */
export function deriveAnchor(image, options = {}) {
...
function opaqueRunsOnRow(image, y) {
  const out = [];
  const rowStart = y * image.width * 4;
  let runStart = -1;
  for (let x = 0; x < image.width; x++) {
...
function pickRunClosestToCenter(runs, centerX) {
  let best = runs[0];
...
export const DERIVE_ANCHOR_DEFAULTS = Object.freeze({
  bandRows: DEFAULT_BAND_ROWS,
  centerToleranceX: DEFAULT_CENTER_TOLERANCE_X,
});
//# sourceMappingURL=derive-anchor.js.map

...
export function decodeSprite(buffer) {
...
const ok = (sensor) => ({ ok: true, sensor });
const fail = (sensor, reason, pixels) =>
  pixels === undefined ? { ok: false, sensor, reason } : { ok: false, sensor, reason, pixels };
export function dimensionsExact(image, brief) {
...
export function alphaBinary(image, opts = {}) {
...
export function paletteMembership(image, palette, opts = {}) {
...
function gatherOpaqueStats(image) {
  let count = 0;
...
export function opaqueBboxFits(image) {
  return opaqueBboxFitsWithOptions(image);
}
export function opaqueBboxFitsWithOptions(image, opts = {}) {
...
function analyzeEdgeTouchingComponents(image, opts) {
...
function extractOpaqueComponents(image) {
...
function isFrameEdge(x, y, width, height) {
  return x === 0 || y === 0 || x === width - 1 || y === height - 1;
}
export function opaqueRatio(image, opts = {}) {
...
export function anchorOpaque(image, brief) {
 ...
 */
export function universalSensors(image, brief, palette) {
...
export function gatherOpaquePixels(image) {
  const out = [];
  for (let y = 0; y < image.height; y++) {
    for (let x = 0; x < image.width; x++) {
      const a = image.data[(y * image.width + x) * 4 + 3] ?? 0;
 ...
 */
export function principalAxisAngleRadians(points) {

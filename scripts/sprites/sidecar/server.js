...
export function buildServer(deps) {
  ...
  const store = deps.store ?? new LocalRunStore(deps.runsDir);
  ...
  app.get('/api/runs/:briefId/:runId', async (req, reply) => {
  ...
  app.get('/api/runs/:briefId/:runId/brief', async (req, reply) => {
    const { briefId, runId } = req.params;
  ...
  app.get('/api/runs/:briefId/:runId/sheet/:filename', async (req, reply) => {
  ...
  app.get('/api/runs/:briefId/:runId/sheets', async (req, reply) => {
    const { briefId, runId } = req.params;
    // safeJoin as segment validator — see /api/runs/:briefId/:runId for rationale.
    if (safeJoin(deps.runsDir, [briefId, runId, 'summary.json']) === null) {
      reply.code(403);
  ...
  app.get('/api/runs/:briefId/:runId/slice-map', async (req, reply) => {
    const { briefId, runId } = req.params;
  ...
  app.get('/api/runs/:briefId/:runId/processed/:filename', async (req, reply) => {
    const { briefId, runId, filename } = req.params;
  ...
  app.get('/api/runs/:briefId/:runId/raw/:filename', async (req, reply) => {
    const { briefId, runId, filename } = req.params;
    const ext = path.extname(filename).toLowerCase();
    if (ext !== '.png') {
      reply.code(415);
  ...
  app.post('/api/runs/:briefId/:runId/approve', async (req, reply) => {
  ...
  app.post('/api/workflow/synthesize', async (req, reply) => {
    const body = req.body ?? {};
    ...
    if (body.type !== undefined) {
      if (typeof body.type !== 'string' || !SPRITE_TYPES.includes(body.type)) {
        reply.code(400);
        return {
          error: 'bad-request',
  ...
  app.post('/api/workflow/promote-brief', async (req, reply) => {
    const body = req.body ?? {};
  ...
  app.post('/api/workflow/generate', async (req, reply) => {
    const body = req.body ?? {};
  ...
  app.post('/api/workflow/metadata', async (req, reply) => {
    const body = req.body ?? {};
    const providerMode = body.provider ?? 'auto';
    ...
    try {
      ...
      const catalog = parseSpriteCatalog(JSON.parse(readFileSync(catalogAbs, 'utf8')));
  ...
  app.post('/api/postprocess', async (req, reply) => {
    const body = req.body ?? {};
    if (typeof body.briefPath !== 'string' || body.briefPath.trim() === '') {
      reply.code(400);
      return { error: 'bad-request', message: 'body.briefPath must be a non-empty string' };
  ...
  app.delete('/api/runs/:briefId/:runId', async (req, reply) => {
    const { briefId, runId } = req.params;
    const runDir = safeJoin(deps.runsDir, [briefId, runId]);
    if (runDir === null) {
      reply.code(403);
 ...
 */
async function listRunsFromStore(store) {
  ...
  for (const key of summaryKeys) {
    ...
    let summary = null;
    try {
      summary = JSON.parse((await store.get(key)).toString('utf8'));
    } catch {

...
export interface RepostprocessArgs {
   ...
   * modules the brief requires to stay disabled. Use when a persisted or
   * explicit `disabledModules` set is itself the reason a run is stuck (e.g. a
   * stale operator override left over from a previous, now-irrelevant salvage
   * attempt) and the operator wants a clean recovery baseline.
   */
...
export async function repostprocessRun(args: RepostprocessArgs): Promise<RerunResult> {
  ...
  // Forced recovery drops any persisted/explicit operator disabledModules
  // (which may themselves be the reason a salvage attempt is stuck) down to
  // just the modules the brief requires — it never re-enables a module the
  // brief itself demands stay off.

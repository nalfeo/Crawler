 ...
 */
function resolveChatDeployment(env, warn) {
 ...
 */
const warnedFallbackDeployments = new Set();
export function __resetChatDeploymentFallbackWarnings() {
  warnedFallbackDeployments.clear();
}
function warnChatDeploymentFallbackOnce(deployment, warn) {
 ...
 */
export function createTextProvider(options = {}) {
  const env = options.env ?? process.env;
...
function createAzureProvider(env, fetchImpl) {
...
function createAzureChatProvider(env, warn, fetchImpl) {
 ...
 */
export function createSynthProvider(options = {}) {
  const env = options.env ?? process.env;
  const warn = options.warn ?? defaultWarn;
  const which = (
...
function createAzureVisionProvider(env, fetchImpl) {

...
export class AzureOpenAISynthProvider {
  endpoint;
  deployment;
  apiKey;
  apiVersion;
 ...
 */
function parseSynthResponse(content, request) {
  ...
  const candidates = candidatesRaw.map((c, idx) => parseCandidate(c, idx));
...
function parseCandidate(value, idx) {
...
function parseInferredType(value) {
  ...
  const found = SPRITE_TYPES.includes(value);
  if (!found) {
    throw new SynthProviderError(
      'malformed',
...
//# sourceMappingURL=azure-chat-synth.js.map

...
export function postprocess(rawPng, brief, palette, options = {}) {
  return postprocessWithTrace(rawPng, brief, palette, options).finalPng;
}
export function postprocessWithTrace(rawPng, brief, palette, options = {}) {
  if (palette.length === 0) {
    throw new Error('postprocess: palette must contain at least one color');
  }
  const steps = [];
  const pushStep = (id, label, image) => {
 ...
 */
export function removeIsolatedNearWhiteSpeckles(image, opts = {}) {
...
export function removeBackground(image, toleranceSq = BACKGROUND_COLOR_TOLERANCE_SQ) {
...
export function removeBackgroundB(image, options = {}) {
...
function removeBackgroundFringe(flooded, source, toleranceSq, clearEnclosedIslands) {
...
function clearEnclosedNearBackgroundIslands(data, width, height, cornerColors, toleranceSq) {
  const total = width * height;
...
function isNearCornerColor(data, idx, cornerColors, toleranceSq) {
  return minCornerColorDistanceSq(data, idx, cornerColors) <= toleranceSq;
}
function minCornerColorDistanceSq(data, idx, cornerColors) {
  const r = data[idx] ?? 0;
...
function getCornerColors(image) {
...
function floodFill(data, visited, width, height, startX, startY, r, g, b, toleranceSq) {
...
function normalizeTolerance(raw, fallback) {
  if (raw === undefined) return fallback;
  if (!Number.isFinite(raw)) return fallback;
  const normalized = Math.round(raw);
  return normalized >= 0 ? normalized : fallback;
}
function fitWithinNearest(image, boxW, boxH, centerSubject) {
 ...
 * Exported for direct unit testing.
 */
export function quantizeToPalette(image, palette) {
  if (palette.length === 0) {
...
function nearestPaletteEntry(r, g, b, palette) {
  let bestIdx = 0;
 ...
 * Exported for direct unit testing.
 */
export function hardThresholdAlpha(image) {
  const { width, height, data: src } = image;
  const dst = new Uint8Array(src);
 ...
 * Exported for direct unit testing.
 */
export function trimTransparentEdges(image) {
  const { width, height, data } = image;
 ...
 * Exported for direct unit testing.
 */
export function scaleToMinDimension(image, minPx) {
  const { width, height } = image;
 ...
 */
function upscaleNearest(image, dstW, dstH) {

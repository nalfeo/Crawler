...
import { judgeVariant } from './judge.js';
...
import { makeRunId, pickChosen, rankCandidates } from './run-artifacts.js';
const logger = createLogger('infra:generate-one');
const RETRYABLE_PROVIDER_KINDS = new Set(['bad-grid', 'non-png']);
export async function generateOne(options) {
  ...
  const store = options.store ?? new LocalRunStore(path.join(outputRoot, 'runs'));
  ...
  const storeKey = (rel) => `${brief.name}/${runId}/${rel}`;
  ...
  for (let i = 0; i < sliced.length; i++) {
    ...
    if (scorecard.derivedAnchor) {
      ...
      await store.put(
        anchorKey,
        Buffer.from(
          `${JSON.stringify({ x: scorecard.derivedAnchor.x, y: scorecard.derivedAnchor.y, source: 'derived' }, null, 2)}\n`,
        ),
      );
  ...
  // surviving the judge.
  const judgePlan = new Map();
  const judgeSkipReason = new Map();
  if (judgeEnabled) {
...
//# sourceMappingURL=generate-one.js.map

...
export class AzureStorageQueue {
  ...
  visibilityTimeout;
  backend = 'azure-queue';
  constructor(client, visibilityTimeout) {
    this.client = client;
   ...
   */
  static fromConnectionString(connectionString, queueName) {
  ...
  async dequeue() {
    ...
    let request;
    try {
      request = JSON.parse(msg.messageText);
    } catch {
  ...
  async peek(maxCount = 1) {
    ...
    for (const msg of response.peekedMessageItems) {
      try {
        results.push(JSON.parse(msg.messageText));
      } catch {
...
//# sourceMappingURL=azure-queue.js.map

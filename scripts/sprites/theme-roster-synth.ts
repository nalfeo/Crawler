...
export function buildRosterSystemPrompt(): string {
  return [
    ...
    '- Each equipment entry lists the slots that one item occupies; most items occupy exactly one.',
...
export function buildRosterUserPrompt(
  ...
  lines.push(
    '',
    `Propose the roster. Aim for ${THEME_EQUIPMENT_SET_MIN_WEAPON_TYPES + 1} weapons across that many distinct weapon types, and one item per valid equipment slot.`,
  );
  if (previousFailure) {
    lines.push(
      '',

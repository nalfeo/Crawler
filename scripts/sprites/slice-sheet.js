 ...
 */
export function computeSliceMap(sheetPng, options) {
 ...
 */
export function sliceSheet(sheetPng, options) {
  const { rows, cols } = options;
  if (rows < 1 || cols < 1) {
...
function findBgColumns(sheet, bg, threshold) {
  const result = new Array(sheet.width).fill(true);
...
function findBgRows(sheet, bg, threshold) {
  const result = new Array(sheet.height).fill(true);
  for (let y = 0; y < sheet.height; y++) {
    for (let x = 0; x < sheet.width; x++) {
      const idx = (y * sheet.width + x) * 4;
      const r = sheet.data[idx] ?? 0;
...
function maskToBands(mask, minWidth) {
  const bands = [];
  let inBand = false;
  let bandStart = 0;
...
function inferContentBounds(sheet, bg, threshold) {
  const colForeground = new Array(sheet.width).fill(0);
  const rowForeground = new Array(sheet.height).fill(0);
 ...
 */
export function computeSliceMapV2(sheetPng, options = {}) {
  ...
  return {
    ...
    cellW,
    cellH,
    rowOffsets: new Array(rows).fill(0),
    colOffsets: new Array(cols).fill(0),
    cells,
  };
}
/**
 ...
 */
export function sliceSheetFromBrief(sheetPng, brief) {
...
function extractCell(sheet, x0, y0, width, height) {
...
function inferRowOffsets(sheet, rows, cellH, maxShift, bgThreshold) {
  const bg = estimateSheetBackgroundRgb(sheet);
  const out = [];
  for (let r = 0; r < rows; r++) {
    const baseY = r * cellH;
...
function inferColOffsets(sheet, cols, cellW, maxShift, bgThreshold) {
  const bg = estimateSheetBackgroundRgb(sheet);
  const out = [];
  for (let c = 0; c < cols; c++) {
    const baseX = c * cellW;
...
function estimateSheetBackgroundRgb(sheet) {
  const corners = [
    [0, 0],
    [sheet.width - 1, 0],
    [0, sheet.height - 1],
...
function rowForegroundCount(sheet, y, bg, threshold) {
...
function colForegroundCount(sheet, x, bg, threshold) {
...
//# sourceMappingURL=slice-sheet.js.map

...
test('findRangeViolations: workspace: specifier passes when exempted for that exact version string', () => {
  // We simulate an exemption by temporarily modifying exports is not practical
  // for internal exemptions; instead test the exported function's behavior via
  // a pkg that has no range entries at all (exemptions are constants).
  // This test validates the function signature is correct when all entries are exact.
  const pkg = { dependencies: { 'my-exact-pkg': '1.0.0' } };
  assert.deepEqual(findRangeViolations(pkg), []);
});

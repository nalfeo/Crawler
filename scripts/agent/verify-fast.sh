...
is_supported_ts_path() {
  [[ "$1" =~ ^(vite\.config\.ts|vitest\.config\.ts|\.github/scripts/.*\.(tsx?|mts|cts)|(src|tests|scripts|tools)/.*\.(tsx?|mts|cts))$ ]]
}
...
if command -v git >/dev/null 2>&1 && git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
  ...
  if [ "${#unsupported_ts[@]}" -ne 0 ]; then
    echo "❌ verify:fast does not support changed TypeScript files outside vite.config.ts, vitest.config.ts, .github/scripts/, src/, tests/, scripts/, and tools/:" >&2
    printf '   - %s\n' "${unsupported_ts[@]}" >&2

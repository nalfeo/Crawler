...
import { FLOOR1_TIME_BUDGET_MS } from './winrate-sweep-args.js';
...
export interface SweepRunClassification {
   ...
   * safe-room-credited active time is under the Floor-1 budget.
   * For non-Floor-1 floors this is identical to `outcomeVictory` (no
   * validated active-time budget on other floors).
   */
  officialWin: boolean;
  /**
 ...
 *
 * Floor 1: official win = victory AND active time < 6-min budget.
 * Other floors: no validated active-time budget, so official win = victory
 * (no run can be slow on those floors by this definition).
 */
export function classifySweepRun(stats: RunStats, floorId: string): SweepRunClassification {
  ...
  const officialWin =
    floorId === 'floor1' ? isOfficialWin(stats, FLOOR1_TIME_BUDGET_MS) : outcomeVictory;

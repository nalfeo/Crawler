...
describe('HudFamilyRelationships deterministic visual guard', () => {
  ...
  it('contains every label and avoids the minimap and adjacent HUD at target viewports', async () => {
    for (const viewport of [
      ...
      expect(rapid.family.rows).toHaveLength(4);
      if (rapid.family.panel && rapid.minimap) {
        expect(overlaps(rapid.family.panel, rapid.minimap)).toBe(false);
      }
      expect(rapid.family.panel && overlaps(rapid.family.panel, rapid.bottomCenter)).toBe(false);
    }
  });
});

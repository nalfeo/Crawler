...
function setupFloor2BossEncounter(seed: number) {
  ...
  return {
    world,
    floorMap,
    encounter: familyState.bossEncounters!.get(target.familyId)!,
...
describe('Floor 2 boss den containment', () => {
  it('returns a strayed boss to its den before the encounter latches the door relock', () => {
    const seed = 7777;
    const { world, floorMap, encounter, familyId } = setupFloor2BossEncounter(seed);

...
describe('release baseline regression workflow', () => {
  ...
  it('does not file an improvement issue for a healthy published release baseline', () => {
    const steps = baselineSteps();
    expect(steps.some((step) => step.name?.includes('nightly balance issue'))).toBe(false);

...
describe('Floor 3 automatic companion controls', () => {
  const source = readFileSync('src/engine/scenes/MainGameScene.ts', 'utf-8');

  it('has no active command button, binding, queued request, or unlock toast', () => {
    expect(source).not.toMatch(
      /floor3Command|keyCommand|queuedCompanionCommand|requestCompanionCommand/,
    );
    expect(source).not.toContain('Command unlocked!');
    expect(source).not.toContain('Phaser.Input.Keyboard.KeyCodes.C);');
  });
});

...
import {
  floor4GreenRoomAffordabilityBudget,
  floor4GreenRoomCheapestOfferPrice,
  floor4StockStreamKey,
  getFloor4GreenRoomCurrentVisit,
  getFloor4GreenRoomState,
  openFloor4GreenRoomVisit,
  rollFloor4GreenRoomVisit,
...
describe('floor4 Green Room stock — derived streams', () => {
  it('rolls each table from the documented per-visit stream key', () => {
    ...
    const visit = rollFloor4GreenRoomVisit(world, 0);
    expect(visit.tables.length).toBeGreaterThanOrEqual(2);
    for (const table of visit.tables) {
      expect(table.streamKey).toBe(floor4StockStreamKey(world.seed, 0, table.tableId));
    }
  });

  it('is deterministic: same seed + visit ⇒ identical stock', () => {
    const a = rollFloor4GreenRoomVisit(createTestWorld({ seed: 7 }), 2);
    const b = rollFloor4GreenRoomVisit(createTestWorld({ seed: 7 }), 2);
    expect(visitSignature(a)).toBe(visitSignature(b));
  });

  it('is path-independent: visit N does not depend on other visits or roll order', () => {
    const forward = createTestWorld({ seed: 99 });
    const backward = createTestWorld({ seed: 99 });
    const forwardVisits: string[] = [];
    for (let v = 0; v < ACT_COUNT; v += 1) {
      forwardVisits[v] = visitSignature(rollFloor4GreenRoomVisit(forward, v));
    }
    for (let v = ACT_COUNT - 1; v >= 0; v -= 1) {
      expect(visitSignature(rollFloor4GreenRoomVisit(backward, v))).toBe(forwardVisits[v]);
    }
  });
  ...
  it('scales prices per visit: later breaks are strictly pricier for the same rolled item', () => {
    ...
    const early = rollFloor4GreenRoomVisit(world, 0);
    const late = rollFloor4GreenRoomVisit(world, ACT_COUNT - 1);
    let compared = 0;
  ...
  it('rejects an out-of-range visit index', () => {
    const world = createTestWorld({ seed: 42 });
    expect(() => rollFloor4GreenRoomVisit(world, -1)).toThrow();
    expect(() => rollFloor4GreenRoomVisit(world, ACT_COUNT)).toThrow();
  });
});
...
describe('floor4 Green Room stock — visit lifecycle', () => {
  it('creates run-scoped state lazily on first open', () => {
    const world = createTestWorld({ seed: 42 });
    expect(getFloor4GreenRoomState(world)).toBeUndefined();
    const result = openFloor4GreenRoomVisit(world, 0);
    expect(result.ok).toBe(true);
    expect(getFloor4GreenRoomState(world)?.lastOpenedVisitIndex).toBe(0);
    expect(getFloor4GreenRoomCurrentVisit(world)?.visitIndex).toBe(0);
  });
  ...
  it('retirement clears the offer and is a no-op when nothing is open', () => {
    ...
    expect(getFloor4GreenRoomCurrentVisit(world)).toBeUndefined();
    expect(getFloor4GreenRoomState(world)?.retiredVisitCount).toBe(1);
  });
});
...
describe('floor4 Green Room stock — affordability invariant (spec §8)', () => {
  it('every visit always offers something at or below its guaranteed gold budget', () => {
    // Budgets are authored above the analytic worst-case cheapest offer, so the
    // invariant must hold for EVERY seed, not merely the sample below.
    for (let seed = 0; seed < 400; seed += 1) {
      const world = createTestWorld({ seed });
      for (let v = 0; v < ACT_COUNT; v += 1) {
        const visit = rollFloor4GreenRoomVisit(world, v);
        const cheapest = floor4GreenRoomCheapestOfferPrice(visit);
        expect(cheapest).toBeLessThanOrEqual(floor4GreenRoomAffordabilityBudget(v));
      }
    }
  });
});

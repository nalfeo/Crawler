...
// ---------------------------------------------------------------------------
// Analytic ring-WALL barriers (sub-tile 1 ft wall — the "smooth circle" path)
// ---------------------------------------------------------------------------

describe('pointInRingBand', () => {
  it('is true on the band, false in the interior and exterior', () => {
    // Band [9, 10] centred at (100, 100).
    expect(pointInRingBand(100, 100, 9, 10, 100, 109.5)).toBe(true); // mid-band
    expect(pointInRingBand(100, 100, 9, 10, 100, 105)).toBe(false); // interior
    expect(pointInRingBand(100, 100, 9, 10, 100, 112)).toBe(false); // exterior
  });

  it('is inclusive on both edges', () => {
    expect(pointInRingBand(0, 0, 9, 10, 9, 0)).toBe(true); // exactly inner
    expect(pointInRingBand(0, 0, 9, 10, 10, 0)).toBe(true); // exactly outer
  });
});

describe('createRingWallBarrier + isBarrierPointBlocked', () => {
  it('owns no tiles but carries an analytic ring shape', () => {
    const world = makeBarrierWorld(makeRoomMap());
    const handle = createRingWallBarrier(world, 40, 40, 10, 1, 'fence');
    expect(handle.tiles).toEqual([]);
    expect(handle.shape).toEqual({
      type: 'ring',
      cxFt: 40,
      cyFt: 40,
      innerRadiusFt: 9,
    ...
    // Registry indexes the shape by handle id for the point lookup.
    expect(world.barriers.ringShapes.get(handle.id)).toBe(handle.shape);
    // blockedTiles stays empty — nothing to rasterize.
    expect(world.barriers.blockedTiles.size).toBe(0);
  });

  it('blocks only the wall band: interior free, band solid, exterior free', () => {
    const world = makeBarrierWorld(makeRoomMap());
    createRingWallBarrier(world, 40, 40, 10, 1, 'fence');
    expect(isBarrierPointBlocked(world, 40, 45)).toBe(false); // r=5, interior
    expect(isBarrierPointBlocked(world, 40, 49.5)).toBe(true); // r=9.5, band
    expect(isBarrierPointBlocked(world, 40, 52)).toBe(false); // r=12, exterior
  });

  it('is a fast no-op (false) when no analytic barriers exist', () => {
    const world = makeBarrierWorld(makeRoomMap());
    expect(world.barriers.ringShapes.size).toBe(0);
    expect(isBarrierPointBlocked(world, 40, 49.5)).toBe(false);
  });

  it('dropping the handle clears the ring shape and re-opens the band', () => {
    const world = makeBarrierWorld(makeRoomMap());
    const handle = createRingWallBarrier(world, 40, 40, 10, 1, 'fence');
    expect(isBarrierPointBlocked(world, 40, 49.5)).toBe(true);
    dropBarrier(world, handle);
    expect(world.barriers.ringShapes.size).toBe(0);
    expect(isBarrierPointBlocked(world, 40, 49.5)).toBe(false);
  });

  it('routes through FloorMap.isPassableAt at feet precision', () => {
    const world = makeBarrierWorld(makeRoomMap());
    createRingWallBarrier(world, 40, 40, 10, 1, 'fence');
    // A point on the band is impassable; a point one foot inside is passable —
    // proving sub-tile resolution (both lie in the same 4 ft tile column).
    expect(world.floorMap!.isPassableAt(40, 49.5)).toBe(false);
    expect(world.floorMap!.isPassableAt(40, 45)).toBe(true);
  });
});

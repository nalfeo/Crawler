 ...
 * dies. The test drives the real map + real simulation-step pipeline (no
 * synthetic fixtures), and uses a range of natural seeds (1..8) — a bug
 * that only cages on lucky geometry would fail on at least one seed.
 *
 * Assertions (per seed):
...
import { Position, Spawner } from '../../src/core/components.js';
...
describe.each(SEEDS)('spawner-arena caging on natural Floor-1 (seed %i)', (seed) => {
  it('physically cages the player once armed; releases on spawner kill', () => {
    ...
    // arenaKind 0 = sealed-room, 1 = open-fence.
    ...
    for (let tick = 0; tick < 240; tick += 1) {
      ...
      // (a) Player never occupies a barrier tile (impenetrability).

...
describe('melee weapons', () => {
  ...
  it('melee weapon does not swing at an in-combat enemy beyond gate range', () => {
    const world = createTestWorld();
    spawnPlayer(world, 12.5, 12.5);
    // Enemy is well within the combat radius (150 ft) so the player counts as
    // "in combat", but far outside the sword gate range (~7.5 ft), so no swing.
  ...
  it('0-damage miss swing does not deal damage when it contacts an enemy', () => {
    const world = createTestWorld();
    spawnPlayer(world, 12.5, 12.5);
    const enemy = spawnEnemy(world, 16.25, 12.5, 50); // within blade length
    const def = getWeaponDef('sword')!;

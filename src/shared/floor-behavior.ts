...
export const floorBehaviorSchema = z
  .object({
    ...
    /**
     * Spawn the ambient trash attack-wave packs (`attackWaveSystem`) on this
     * floor. Layered under `GameWorld.attackWaveFlags.attackWaves`, which stays
     * the run-local kill switch: the waves only run when the floor opts in *and*
     * the run enables them.
     */
    trashAttackWaves: z.boolean().default(false),
  })
  .strict();

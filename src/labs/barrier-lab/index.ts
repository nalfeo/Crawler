...
function createBarrierLab(canvasHost: HTMLElement, controls: HTMLElement): () => void {
  ...
  function render(): void {
    ...
    // Analytic ring-wall shapes render at feet precision (1 ft == 1 px in this
    // lab), so a sub-tile 1 ft wall shows as a smooth annulus rather than a
    // staircase of blocked tiles.
    for (const h of world.barriers.barriers.values()) {
      const shape = h.shape;
      if (!shape || shape.type !== 'ring') continue;
      const midR = (shape.innerRadiusFt + shape.outerRadiusFt) / 2;
      const thick = Math.max(1, shape.outerRadiusFt - shape.innerRadiusFt);
      g.strokeStyle = KIND_COLOR[h.kind];
      g.globalAlpha = 0.85;
      g.lineWidth = thick;
      g.beginPath();
      g.arc(shape.cxFt, shape.cyFt, midR, 0, Math.PI * 2);
      g.stroke();
      g.globalAlpha = 1;
    }
    ...
    status.textContent = [
      ...
      `ring-wall shapes: ${world.barriers.ringShapes.size}`,
  ...
  gui.add(settings, 'wallThicknessFt', 1, 40, 1).name('Wall thickness (ft)');
  ...
  gui
    ...
    .name('Create ring WALL (analytic)');
  ...
  hint.innerHTML =
    'Ring <em>WALL</em> barriers cage a disc with a thin analytic annulus (sub-tile precision, drawn as a smooth circle — this is what spawner arenas use). Ring barriers cage a disc via blocked tiles. Doorway barriers plug every doorway in a room (belt-and-suspenders alongside door locks). Poly barriers accept any tile set (turn on <em>Poly draft mode</em> and click tiles).';

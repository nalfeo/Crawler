...
interface ReprocessProfileInput {
  name: string;
  tuning?: {
    minChannel?: number;
...
interface ReprocessRequestBody {
  ...
  sourceRunId: string;
  briefPath?: string;
  profileA: ReprocessProfileInput;
  profileB?: ReprocessProfileInput;
}
...
export async function postReprocess(
  request: ReprocessRequestBody,
  fetcher: typeof fetch = fetch,
): Promise<ReprocessResponse> {
  const res = await fetcher(`${SIDECAR_BASE}/api/workflow/reprocess`, {
    method: 'POST',
  ...
  if (!res.ok) {
    ...
    throw new Error(`reprocess failed (${res.status}): ${detail || res.statusText}`);
  }
  return (await res.json()) as ReprocessResponse;
}
...
function createGalleryLab(canvasHost: HTMLElement, controls: HTMLElement): () => void {
  ...
  function renderSidePanel(ref: CandidateRef | null): void {
    ...
    reprocessSection.append(
      el('div', {
        textContent: 'Postprocess A/B',
        style: { fontWeight: '600', fontSize: '12px', color: '#f1f5f9', marginBottom: '6px' },
      }),
    );
    const modeOptions: ReadonlyArray<{
      value: 'edge-drop' | 'preserve-orphans' | 'disabled';
      label: string;
    }> = [
      { value: 'edge-drop', label: 'edge-drop' },
    ...
    const profileAName = el('input', {
      value: 'edge-drop',
      placeholder: 'A label',
      style: inputStyle,
    });
    const profileAMode = el('select', { style: inputStyle });
    for (const option of modeOptions) {
      profileAMode.append(el('option', { value: option.value, textContent: option.label }));
    }
    profileAMode.value = 'edge-drop';
    const enableB = el('input', {
      type: 'checkbox',
      checked: true,
      style: { margin: '0 6px 0 0' },
    });
    const profileBName = el('input', {
      value: 'preserve',
      placeholder: 'B label',
      style: inputStyle,
    });
    const profileBMode = el('select', { style: inputStyle });
    for (const option of modeOptions) {
      profileBMode.append(el('option', { value: option.value, textContent: option.label }));
    }
    profileBMode.value = 'preserve-orphans';
    const setBEnabled = (enabled: boolean): void => {
      profileBName.disabled = !enabled;
      profileBMode.disabled = !enabled;
      profileBName.style.opacity = enabled ? '1' : '0.6';
      profileBMode.style.opacity = enabled ? '1' : '0.6';
    };
    setBEnabled(enableB.checked);
    enableB.addEventListener('change', () => setBEnabled(enableB.checked));
    const runReprocessBtn = el('button', {
      type: 'button',
      textContent: 'Run reprocess',
      style: {
        padding: '6px 10px',
        borderRadius: '6px',
        border: '1px solid rgba(148,163,184,0.4)',
    ...
    const reprocessStatus = el('div', {
      textContent: 'Run postprocess-only A/B from this run.',
    ...
    runReprocessBtn.addEventListener('click', async () => {
      const selectedSnapshot = state.selected
        ...
        : null;
      runReprocessBtn.disabled = true;
      reprocessStatus.style.color = '#93c5fd';
      reprocessStatus.textContent = 'Reprocessing…';
      try {
        const profileA: ReprocessProfileInput = {
          name: profileAName.value.trim() || 'A',
          modules: {
            speckleMode: profileAMode.value as 'edge-drop' | 'preserve-orphans' | 'disabled',
          },
        };
        const profileB: ReprocessProfileInput | undefined = enableB.checked
          ? {
              name: profileBName.value.trim() || 'B',
              modules: {
                speckleMode: profileBMode.value as 'edge-drop' | 'preserve-orphans' | 'disabled',
              },
            }
          : undefined;
        const result = await postReprocess({
          sourceBriefId: run.briefId,
        ...
        if (result.runs.length > 0) {
          ...
          const briefIndex = state.runs.findIndex(
            (entry) => entry.briefId === latest.briefId && entry.runId === latest.runId,
          );
          if (briefIndex >= 0) {
            selectCandidate({ briefIndex, candidateIndex: 0 });
          } else if (selectedSnapshot) {
            state.selected = selectedSnapshot;
          }
        }
        reprocessStatus.style.color = '#86efac';
        reprocessStatus.textContent = `Created runs:\n${result.runs
          .map((created) => `${created.profile}: ${created.briefId}/${created.runId}`)
          .join('\n')}`;
      } catch (error) {
        reprocessStatus.style.color = '#fecaca';
        reprocessStatus.textContent = error instanceof Error ? error.message : String(error);
      } finally {
        runReprocessBtn.disabled = false;
      }
    });
    reprocessSection.append(
      el('div', { style: rowStyle }, [profileAName, profileAMode]),
      el(
        'label',
        {
          style: {
        ...
        [enableB, 'Enable B profile'],
      ),
      el('div', { style: rowStyle }, [profileBName, profileBMode]),
      runReprocessBtn,
      reprocessStatus,
    );
    sidePanel.append(reprocessSection);

 ...
 *
 * Visual debugging and monitoring of the multi-track behavior-tree AI. Renders
 * the live BehaviorTreeAI driving a real Floor 1 scene and surfaces both tracks
 * of the parallel root:
 *
 ...
 *   > Hunt > Explore). Owns the decision state shown at the top of the panel.
 * - Track B (Opportunistic): ticks every frame and writes pull/dodge vectors
 *   blended additively into Track A's direction.
 *
 * The controls panel shows:
 * - Current Track A state and reason (+ target)
 * - Live Track B vector values (collect pull, farm pull, dodge)
 * - A compass overlay: raw Track A direction vs. the final blended direction
 * - The serialised tree structure with Track A / Track B highlighted
 */
...
// Track B blend weights (mirror BehaviorTreeAI defaults) used only for the
// compass overlay arrow lengths.
const COLLECT_PULL_WEIGHT = 0.25;
const DODGE_WEIGHT = 0.4;
...
function buildControlsPanel(controls: HTMLElement): {
  ...
  header.innerHTML =
    '<h3 style="margin:0;color:#fff;">Behavior Tree Visualization</h3>' +
...
registerLab('bt-viz', {
  ...
  description:
    'Visualize and debug the multi-track AI behavior tree: Track A (movement goal) priority selector + Track B (opportunistic collect/dodge/farm) overlay',
  category: 'Meta',
  ...
  create: (canvas: HTMLElement, controls: HTMLElement) => {
    ...
    // Custom AI input provider that drives the player from the behavior tree.
    ...
    // Render tree structure once (the structure is static; node states change).

/**
 * HudFloor3Party — Floor-3 party HUD (game-design §15 surfaces 4, 6, 8).
 *
 * One row per party Companion: slot chip, form name + level, HP bar, KO tag,
 * affinity swatch, fighting-style glyph, and engagement matchup chevron.
 * Below the rows a transient notice strip announces level-ups, evolutions,
 * and newly learned abilities.
 *
 * All view state comes from the pure resolvers in `floor3-*-state.ts`; this
 * module only draws them, so mounting the HUD can never perturb the simulation.
 *
...
/** Leave the left corner-button stack clear at its maximum mobile scale. */
const PANEL_MARGIN_LEFT = 224;
...
export function createHudFloor3Party(
  ...
  for (let i = 0; i < MAX_ROWS; i += 1) {
    ...
    container.add([background, swatch, name, hpTrack, hpFill, statusText, matchupText]);
    root.add(container);
    rowVisuals.push({
      container,
      background,
  ...
  const detachCrispText = applyCrispText(
    scene,
    [title, ...noticeTexts, ...rowVisuals.flatMap((r) => [r.name, r.statusText, r.matchupText])],
  ...
  function fingerprintFor(
    ...
    return `${rowParts.join('|')}|${noticeLines.join('~')}`;
  }
  ...
  function sync(world: GameWorld, playerEid: number): void {
    ...
    const fingerprint = fingerprintFor(rows, matchups, noticeLines);

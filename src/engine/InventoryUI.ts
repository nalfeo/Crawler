...
export function createInventoryUI(
  ...
  const handleWheel = (
    ...
    const gridBounds = new Phaser.Geom.Rectangle(panelX, gridY, panelWidth, gridHeight);
    if (!Phaser.Geom.Rectangle.Contains(gridBounds, pointer.x / s, pointer.y / s)) return;
    scroll(deltaY > 0 ? 1 : -1);
  };

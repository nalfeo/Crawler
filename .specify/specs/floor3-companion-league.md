...
> **Slice 8/9 scope boundary (landed 2026-08-23):** the `floor3-biomes` map generator
> (`tryGenerateFloor3BiomeOverworld` in `cave-system.ts`) only carves `RoomRole.TERRITORY` +
> `RoomRole.SPAWN` rooms — it never stamps a `BOSS_DEN`/`RESOURCE_HEART` room or
> `BOSS_STAIR_FLOOR` terrain, unlike the floor2-families layout. Slice 8 therefore implements the
> "sealed den" gate as a **deferred/lazy spawn** rather than a physical door-lock: Studios spawn
> immediately into the 7 existing `TERRITORY` zones, and the Final Four roster is not created in
> the world at all until the Studios-defeated counter reaches 6 (`spawnFloor3FinalFourRoster`,
> triggered from `floor3ObjectiveTick`), at a computed "arena" tile nearest the map center. The
> exit staircase pops at the player's existing spawn point rather than a heart-room tile. Building
> the physical Studio dens and Final Four arena set-pieces remains slice 9's deliverable — slice 8
> intentionally does not touch the map generator.
...
| 8     | **Studios + Final Four + seeded variety + objective tick** ✅ _landed 2026-08-23_ | 🍎🍎🍎🍎    | `SeededRandom` Studio/Final-Four selection, roster spawner, `floor3ObjectiveTick` (per-Studio defeat → soft-gated deferred Final-Four spawn → victory/stair-pop), determinism test. Physical Studio dens/Final-Four arena rooms are **not** built here — see slice 9 note below. | after 6,7 |

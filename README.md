...
Crawler uses [Goobers](https://q:/src/Goobers) to run its manual-only `crawler-feature-pr`
...
| `GOOBERS_GITHUB_TOKEN` | Optional     | Repo credential Goobers uses for issue/PR/branch operations (claiming issues, pushing branches, opening PRs).                                                                                                          | Falls back to the workflow's own `github.token` (the built-in `GITHUB_TOKEN`) if unset. Only set this explicitly if you need broader scopes than the default token grants, e.g. a fine-grained PAT with Contents/Issues/Pull-requests read-write on this repo.                                                                                                      |

...
| `CRAWLER_CI_PAT`       | **Required** | Existing repo CI credential Goobers uses for issue/PR/branch operations (claiming issues, pushing branches, opening PRs). `crawler-feature-pr` declares `repo:push`, which the built-in `GITHUB_TOKEN` cannot safely satisfy because a `GITHUB_TOKEN`-authored push would not trigger the normal CI workflow. | The existing repository secret, with **Contents, Issues, and Pull requests: Read and write** on this repo. No additional `GOOBERS_GITHUB_TOKEN` secret is needed.                                                                                                                                                                                                   |

...
function checkIndexMdNotModified(files) {
  const hasIndex = files.some((f) => INDEX_MD_RE.test(f));
  if (!hasIndex) return null;
  return `docs/knowledge/handoffs/INDEX.md must not be committed to a PR branch. INDEX.md is auto-generated by docs-update.yml after each merge to main and must stay off PR branches — concurrent agents modifying it is the primary source of merge-queue serialization (see issue #1856). To restore it: \`git restore --source=origin/main --staged --worktree docs/knowledge/handoffs/INDEX.md\` then re-commit. (Exception: the automation/docs-update branch is the only intended path for INDEX.md updates.)`;
}

function evaluatePreflightChecks({ files, addedFiles, cwd, warnings = [] }) {
  const denyParts = [];

  const indexIssue = checkIndexMdNotModified(files);

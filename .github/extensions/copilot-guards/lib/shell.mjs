...
const WRAPPER_RE =
  /^\s*(?:bash|sh|pwsh|powershell)(?:\.exe)?(?:\s+-[A-Za-z][\w-]*)*\s+(?:-c\b|-Command\b|-lc\b|-ic\b|-lic\b|-li\b)\s+/i;
...
export function normalizeCommand(raw) {
  ...
  return rawSegments.map((seg) => seg.trim().replace(WHITESPACE_RE, ' ')).filter(Boolean);
}

...
function renderHtml(state) {
  const { sheetB64, rows, cols, frameRate, name, outputW, outputH } = state;
...
(function() {
  ...
  img.onload = function() {
    ...
    for (let i = 0; i < TOTAL_FRAMES; i++) {
      ...
      fc.width = displaySize; fc.height = displaySize;
      fc.style.width = displaySize + 'px'; fc.style.height = displaySize + 'px';
      const ctx = fc.getContext('2d');
      ctx.drawImage(img, c * cellW, r * cellH, cellW, cellH, 0, 0, displaySize, displaySize);
    ...
    function resize() {
      animCanvas.width = displaySize * zoom;
      animCanvas.height = displaySize * zoom;
      animCanvas.style.width = (displaySize * zoom) + 'px';
      animCanvas.style.height = (displaySize * zoom) + 'px';
    }
    ...
    function draw() {
      ...
      ctx.drawImage(img, c * cellW, r * cellH, cellW, cellH,
                    0, 0, displaySize * zoom, displaySize * zoom);
...
async function startServer(instanceId, state) {
  const server = createServer((req, res) => {
    ...
    res.end(renderHtml(servers.get(instanceId)?.state ?? state));
  });
  ...
  canvases: [
    createCanvas({
      ...
      actions: [
        {
          ...
          handler: async (input, ctx) => {
            ...
            try {
              const b64 = loadSheet(input.sheetPath);
              entry.state = {
                sheetB64: b64,
                rows: input.rows,
                cols: input.cols,
      ...
      open: async (ctx) => {
        ...
        if (input.sheetPath) {
          try {
            sheetB64 = loadSheet(input.sheetPath);
          } catch {
            /* show empty state */
          }
        }

...
| [`perf-optimizer`](perf-optimizer/SKILL.md)                                               | Hunt gameplay-neutral resource optimizations (frame time, load time, memory) and prove the simulation is byte-identical via `npm run perf:fingerprint`. Paired with the `perf-optimizer` agent.                      |

...
async function dispatchWorkflow(workflow, inputs, { phase = null } = {}) {
  if (!live) {
    process.stdout.write(`dry-run would-dispatch workflow=${workflow} pr=#${prNumber}\n`);
    return;
  }
  await assertExpectedMetadataUnchanged(phase);
...
if (
  ...
  await dispatchWorkflow(
    'auto-rebase-prs.yml',
    {
      ...
      expected_base_ref: pr.base?.ref || 'main',
      trigger: 'ci-recovery-conflict',
    },
    { phase: 'dispatch-auto-rebase' },
  );

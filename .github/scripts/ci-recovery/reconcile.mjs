...
// The reply must name the repair commit created after dispatch. Baking in headSha
// would name the pre-repair commit, which markerNamesHead() later accepts as an ancestor.
...
const taskBody = [
  ...
  ...(pendingHumanApproval
    ? [
        `> **⚠ Human-approval gate is active.** The \`human-approval-required\` label means a human must approve before this PR can **merge**. That gate applies to the **merge step only**. You MUST still fix every blocker below, push a consolidated repair commit to the PR branch, and post ${postPushMarkerReply} replies in each thread. Do NOT skip repairs or thread replies because of the human-approval label.`,
        '',
      ]
    : []),
  ...
  '',
  `**Review-thread protocol:** For every listed review thread, invoke a separate review agent using a model different from your primary model to validate whether the comment is still applicable to the current head. Fix valid findings. Resolve only deterministic non-applicability (outdated/removed line or file, duplicate already addressed) or a validated ${postPushMarkerReply} result. For substantive disagreement, reply with the validator evidence and leave the thread unresolved for escalation.`,
  '',
  `A top-level PR comment is never sufficient for a review-thread blocker; post the ${postPushMarkerReply} reply in the exact thread comment listed above.`,
  '',
  `When a thread is addressed, run \`git rev-parse HEAD\` immediately after pushing the consolidated repair commit, replace \`<post-push-sha>\` in ${postPushMarkerReply} with that full SHA, then use \`reply_to_comment\` with the **Reply target comment ID** listed above for that thread (not the ID of this task comment). Do not use the dispatch-time head SHA, which identifies the pre-repair commit. The CI recovery reconciler will resolve the review thread automatically on its next pass. Do **not** reply to this task comment to record addressed status — a marker reply on the review-thread comment is the only form recognised by the reconciler. When a thread is deterministically non-applicable (the finding does not apply to the current code and no fix is needed), use \`reply_to_comment\` with body \`✅ Not applicable: <one-line reason>\` instead — do NOT use this for substantive disagreements. Run the repository-required verification and push one consolidated repair commit.`,
].join('\n');

...
for (const thread of unresolvedThreads.filter((candidate) => {
  ...
  // Skip threads that already carry a definitively stale marker SHA — those
  // need the stale-marker hint path below, not a new auto-posted marker.
  const candidateComments = candidate.comments?.nodes ?? [];
  const lastMarkerSha = extractAddressedMarkerSha(
    candidateComments[candidateComments.length - 1]?.body,
  );
...
const taskBody = [
  ...
  '',
  `When a thread is addressed, use \`reply_to_comment\` with the **Reply target comment ID** listed above for that thread (not the ID of this task comment) and set the body to ${ADDRESSED_MARKER_REPLY} (the \`in <sha>\` part is required — omitting the SHA causes the reconciler to ignore the marker). The CI recovery reconciler will resolve the review thread automatically on its next pass. Do **not** reply to this task comment to record addressed status — a marker reply on the review-thread comment is the only form recognised by the reconciler. Run the repository-required verification and push one consolidated repair commit.`,
].join('\n');

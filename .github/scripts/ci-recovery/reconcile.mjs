...
//
// This detection runs BEFORE the outdated-marker posting loop so that threads
// with a stale trusted marker are excluded from auto-resolution via the outdated
// path — they must be re-addressed with the current head SHA instead.
const staleAddressedMarkerByThread = new Map();
for (const thread of unresolvedThreads) {
  // Skip threads that already have a valid resolution marker.
...
// Threads with a stale trusted marker are excluded — they need re-addressing, not
// auto-resolution via the outdated path.

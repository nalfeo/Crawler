...
if (terminalRow.action === DISPATCH_ACTION.WAIT_ADMISSION) {
  ...
  const taskBody = [
    ...
    '',
    `**Review-thread protocol:** For every listed review thread, invoke a separate review agent using a model different from your primary model to validate whether the comment is still applicable to the current head. Use an available validator model from this set: \`claude-sonnet-4.6\`, \`gpt-5.4\`, \`gpt-5.3-codex\`, \`gemini-3.1-pro-preview\` (do not use \`claude-sonnet-4.5\`). Fix valid findings. Resolve only deterministic non-applicability (outdated/removed line or file, duplicate already addressed) or a validated ${POST_PUSH_ADDRESSED_MARKER_REPLY} result. For substantive disagreement, reply with the validator evidence and leave the thread unresolved for escalation.`,
    '',

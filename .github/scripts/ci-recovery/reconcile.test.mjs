...
test('reconcile auto-resolves unresolved outdated threads as deterministic non-applicable', async (t) => {
      ...
      body: gqlReviewThreads([
        {
          ...
          comments: {
            nodes: [
              {
                body: 'Needs update after refactor.',
                authorAssociation: 'MEMBER',
                author: { login: 'reviewer' },
              },
            ],
  ...
  assert.match(
    stdout,
    new RegExp(
      `would-resolve thread=${outdatedThreadId} reason=deterministic-non-applicable-outdated`,
    ),
  );
  assert.doesNotMatch(stdout, new RegExp(`would-resolve thread=${activeThreadId}`));
  assert.deepEqual(mutatingCalls, [], 'dry-run must not issue any mutating API calls');
});

...
test('train mode writes rebase state but fails closed before dispatch if metadata drifts after state persistence', async (t) => {
  const retargetedBase = 'release';
  let pullFetches = 0;
  const { server, port, mutatingCalls } = await startServer({
    [`GET /repos/${OWNER}/${REPO}/pulls/${PR_NUM}`]: () => {
      pullFetches += 1;
      const baseRef = pullFetches < 3 ? 'main' : retargetedBase;
      return {
        body: {
          ...basePr(),
  ...
  assert.match(
    stdout,
    new RegExp(
      `skip pr=#${PR_NUM} reason=base-ref-moved-before-mutation phase=dispatch-auto-rebase expected=main actual=${retargetedBase}`,
    ),
  );
  assert.equal(
    pullFetches,
    3,
    'dispatch fencing must re-fetch metadata after persisting rebase state',
  );
  assert.equal(
    mutatingCalls.filter(
      (call) =>
    ...
    1,
    'the rebase state comment should persist before the dispatch-phase fence trips',
  );
  assert.equal(
    mutatingCalls.filter(
      (call) =>
    ...
    0,
    'metadata drift before dispatch must prevent the workflow dispatch',
  );
});

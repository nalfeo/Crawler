...
test('outdated stale-marker thread keeps the stale-marker recovery hint', async (t) => {
  ...
  // Even though the thread is outdated, the definitive stale SHA must win over
  // outdated auto-resolution so recovery verifies the fix on the current head.
  ...
  assert.ok(taskCommentCall, 'expected a task comment for the stale addressed marker');
  assert.match(
    taskCommentCall.body.body,
    new RegExp(`Stale marker:.*${staleMarkerSha}`),
    'expected the task comment to identify the unreachable marker SHA',
  );
});

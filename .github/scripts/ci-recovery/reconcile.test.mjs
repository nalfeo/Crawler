...
test('converged PR (ARM_AUTO_MERGE) must not close loop incident when arm-auto-merge metadata guard fails', async (t) => {
  ...
  assert.equal(pullFetches, 3, 'state-comment and arm-auto-merge guards must re-fetch live metadata');
  const closeCall = mutatingCalls.find(
    (call) =>
      call.method === 'PATCH' && call.url === `/repos/${OWNER}/${REPO}/issues/502`,
  );
  assert.equal(closeCall, undefined, 'must not PATCH-close the loop incident on metadata-guard skip');
});

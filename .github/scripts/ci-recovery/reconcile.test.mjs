...
test('reconcile auto-resolves outdated threads without requiring a marker reply', async (t) => {
  // Regression: an isOutdated:true thread was added as a blocker (line 1264)
  // but never auto-resolved (shouldResolveThread required a trusted marker).
  // This caused recovery to stall: @copilot was dispatched but had nothing
  // actionable (the code was already fixed), so it never posted a reply,
  // the fingerprint never changed, and the attempt counter exhausted.
  // Fix: shouldResolveThread returns true for any isOutdated:true thread.
  const outdatedThreadId = 'thread-outdated';
        ...
        body: gqlReviewThreads([
          {
            ...
            isResolved: false,
            isOutdated: true,
            path: 'briefs/weapons/tower-spear.yaml',
            line: 23,
            comments: {
              nodes: [
                {
                  id: 'comment-outdated',
                  body: '`minVariations: 6` is inconsistent with the 3 listed `variations`.',
                  author: { login: 'copilot-pull-request-reviewer' },
                  authorAssociation: 'NONE',
                  url: `https://github.com/${OWNER}/${REPO}/pull/${PR_NUM}#discussion_r9999`,
                },
              ],
            },
          },
              ...
              nodes: [
                {
                  ...
                  author: { login: 'copilot-pull-request-reviewer' },
                  authorAssociation: 'NONE',
                  url: `https://github.com/${OWNER}/${REPO}/pull/${PR_NUM}#discussion_r8888`,
                },
              ],
            },
          },
  ...
  // The outdated thread must be resolved via GraphQL mutation.
  const resolutionMutations = mutatingCalls.filter(
    (call) =>
      call.method === 'GRAPHQL_MUTATION' &&
      String(call.body?.query || '').includes('resolveReviewThread'),
  );
  assert.equal(resolutionMutations.length, 1, 'expected exactly one thread resolution mutation');
  assert.equal(
    resolutionMutations[0].body.variables.threadId,
    outdatedThreadId,
    'must resolve the outdated thread',
  );
  assert.match(stdout, new RegExp(`resolved thread=${outdatedThreadId}`));
  assert.doesNotMatch(stdout, new RegExp(`resolved thread=${currentThreadId}`));

  // The current (non-outdated) thread is still a blocker — recovery task dispatched.
  const taskCommentCall = mutatingCalls.find(
    (call) =>
      call.method === 'POST' &&
      call.url === `/repos/${OWNER}/${REPO}/issues/${PR_NUM}/comments` &&
  ...
  assert.ok(taskCommentCall, 'current unresolved thread should still produce a recovery task');
  assert.ok(
    taskCommentCall.body.body.includes(currentThreadId) ||
      taskCommentCall.body.body.includes('src/foo.ts'),
    'recovery task should reference the remaining current thread',
  );
});

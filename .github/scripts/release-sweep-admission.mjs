...
export function decideReleaseSweep({
  ...
  const constrained = demand > maxCompetingDemand;
  const lastSweepMs =
  ...
  if (!constrained) {
    return {
      ...
      constrained,
      hoursSinceLastSweep,
      reason: `runner pool has headroom (competing demand ${demand} <= ${maxCompetingDemand})`,
    };
  }
  if (hoursSinceLastSweep === null) {
    return {
      ...
      constrained,
      hoursSinceLastSweep,
      reason: `runner pool is constrained (demand ${demand}) but no previous baseline was found; sweeping`,
    };
  }
  if (hoursSinceLastSweep >= minIntervalHours) {
    return {
      ...
      constrained,
      hoursSinceLastSweep,
      reason: `runner pool is constrained (demand ${demand}) but the last baseline is ${hoursSinceLastSweep.toFixed(1)}h old (>= ${minIntervalHours}h); sweeping anyway`,
    };
  }
  return {
    shouldSweep: false,
    constrained,
    hoursSinceLastSweep,
    reason: `runner pool is constrained (competing demand ${demand} > ${maxCompetingDemand}; the sweep alone claims up to ${RELEASE_SWEEP_PEAK_RUNNERS} runners) and the last baseline is only ${hoursSinceLastSweep.toFixed(1)}h old (< ${minIntervalHours}h); skipping`,
  };
}
...
export async function resolveReleaseSweepAdmission({
  ...
  return { ...decision, nonSweepJobs, latentBacklog, lastSweepAt, warnings };
}

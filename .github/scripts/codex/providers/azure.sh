...
if [[ -n "${CODEX_EXEC_COMMAND:-}" ]]; then
  ...
  # Auth: prefer an explicit key. Otherwise, if `az` is logged in and the
  # resource coordinates are provided, fetch the key at runtime so no secret has
  # to be stored (keyless-from-the-operator's-view local runs via `az login`).
  if [[ -z "${AZURE_OPENAI_API_KEY:-}" ]]; then
    if command -v az >/dev/null 2>&1 && [[ -n "${AZURE_OPENAI_RESOURCE:-}" && -n "${AZURE_OPENAI_RESOURCE_GROUP:-}" ]]; then
      ...
      # Run the fetch inside the `if` condition so `set -e` does not abort the
      # script before the friendly guidance below when `az` fails (expired
      # login, wrong subscription, RBAC denied, etc.).
      if ! AZURE_OPENAI_API_KEY="$(az cognitiveservices account keys list \
        -n "$AZURE_OPENAI_RESOURCE" -g "$AZURE_OPENAI_RESOURCE_GROUP" --query key1 -o tsv)"; then

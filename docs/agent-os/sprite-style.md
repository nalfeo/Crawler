...
> 10. **Characters and enemy/mob figures must show the FULL BODY — head, torso, and feet all visible in the frame.** A bust shot, portrait crop, or figure cut off at the waist or mid-torso is a hard failure. Do not fill the cell with a close-up face or shoulders. A pure 90-degree side silhouette (full side profile with no face visible) is also a hard failure — mobs should face the camera at roughly a one-third-to-two-thirds turn unless the brief explicitly specifies a different orientation.
>
> --- END STYLE PREAMBLE ---

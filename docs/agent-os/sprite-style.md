...
> 10. **Characters and enemy/mob figures must show the FULL BODY — from the highest visible extent to the lowest visible extent all in frame.** A bust shot, portrait crop, or figure cut off at the waist or mid-body is a hard failure. For upright humanoid figures, head/torso/feet should all be visible when that anatomy is present. Do not fill the cell with a close-up face or shoulders. A pure 90-degree side silhouette (full side profile with no face visible) is also a hard failure — front-facing briefs must keep mobs straight toward camera, while three-quarter/left/right briefs may bias orientation but still keep face-visible camera-facing turn.
>
> --- END STYLE PREAMBLE ---

...
- **Plan-first.** Before writing any code, output your **full plan** in the
  ...
  PR description. Issue comments are allowed when requested (for example,
  posting a pre-PR implementation plan on the source issue). Do not hide plans
  in repo files unless the human explicitly asks for a file artifact.

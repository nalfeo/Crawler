...
- `node --test .github/scripts/merge-train/*.test.mjs` → 158/158 pass (new
  ...
  recheck, and the extracted `planLandedRecovery` decision added in subsequent
  review-fix passes).
- `npm run test:guards` (the CI gate) → 864 pass / 21 skipped / 0 fail.

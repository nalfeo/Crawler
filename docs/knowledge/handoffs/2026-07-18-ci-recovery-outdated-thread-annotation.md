...
1. **`reconcile.mjs`** — Before collecting blockers, build a `threadOutdated: Map<threadId, boolean>` from `thread.isOutdated`. In the task body `flatMap`, look up this map and append ` (outdated)` to the thread header line when it is `true`. Updated the review-thread protocol text to:
   - Explicitly explain the `(outdated)` marker and that such threads must be classified immediately as `deterministically-inapplicable` without a code-review sub-agent call.
...
2. **`ci-review-validator.agent.md`** — Added an explicit protocol step (step 1) for `(outdated)` threads: skip sub-agent invocation, classify as `deterministically-inapplicable`, reply with evidence that code context no longer exists at the specified location.
...
- Ran `npm run verify:fast`: **4254/4255 pass** (1 pre-existing shallow-clone failure in `epic-status.test.ts`, unrelated).

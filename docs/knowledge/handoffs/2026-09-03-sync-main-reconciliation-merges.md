...
Updated `scripts/agent/sync-main.mjs` so ordinary clean authoring branches still use the existing rebase strategy, while shepherd/recovery ownership branches that already contain mainline reconciliation merges use a merge-preserving `origin/main` update. The command now reports the selected strategy and deterministic reason, and the local sync evidence records the strategy fields for later inspection.

## Files touched

- `scripts/agent/sync-main.mjs` — added deterministic strategy selection, short-circuiting mainline reconciliation-merge detection, merge-preserving update/abort handling, and strategy reporting.
- `scripts/agent/sync-main.test.mjs` — added branch-history tests for repeated main advancement, preservation of prior semantic conflict resolutions, merge-preserving conflict aborts, and unchanged ordinary-branch rebase behavior.

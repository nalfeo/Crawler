...
`2026-07-15-merge-train-sibling-stranding-fix.md` equivalent for #1156, and
...
9. Dispatched `Merge Train` (`workflow_dispatch`) — **atomic promotion succeeded**:
   ...
   "was not recorded as merged" failure (the gap-5 bug this session exists to prove
   fixed).
10. Verified postconditions directly against GitHub:
    ...
    - `gh pr view 1131 --json state,closed,mergedAt,mergeCommit` →
      `state: MERGED`, `mergedAt` set, `mergeCommit.oid` == the same sha, `merge-train`
      label cleared (queue drained).
    - Note: GitHub's `merged`/`mergedAt` fields _were_ populated promptly in this
      ...
      worst case, which is consistent with (not contradictory to) the original
      finding that `merged` is _unreliable_, not that it is _always_ false.

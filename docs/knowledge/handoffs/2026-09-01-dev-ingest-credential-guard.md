...
policy (5🍎 → two reviews), `claude-opus-4.6` and `gpt-5.4`. All findings were
fixed before publishing:

- **Critical** (`claude-opus-4.6` + `gpt-5.4`, same root cause): the
  `preflight` step's missing-secrets branch was missing a closing `fi` for
  its `if [ "${{ github.event.inputs.skip_deploy }}" ... ]` check, which
  would have made the step's bash script fail to parse at all. Fixed.
- **High** (`gpt-5.4`): `infra/dev-build-ingest.bicep`'s `githubCiPat`
  parameter had no length constraint, so a workflow secret that resolves to
  an empty string (e.g. a repo secret that exists but was never given a
  value) would satisfy "parameter supplied" while still deploying a Function
  with an unusable credential. Fixed with `@minLength(1)`; covered by a new
  `dev-build-ingest-bicep.test.ts` case ("rejects an empty-string
  githubCiPat, not just an omitted one").
- **Medium** (`gpt-5.4`): the `canary` job's original `if:` condition could
  suppress the canary on a specific partial-deploy-failure shape. Simplified
  to unconditional `if: always()` — the canary must run regardless of
  `deploy`/`preflight` outcome.
- **Medium** (`gpt-5.4`): `run-canary.mjs` could misreport a GitHub API
  failure that happens _after_ a successful issue-filing (e.g. failing to
  label/close the canary issue) as a broken-credential alert — a false
  positive that would mask the real signal. Fixed by splitting `runCanary()`
  into two stages: a POST-and-parse-issue-URL failure sets
  `credentialSuspected: true` and files the alert; a later cleanup-only
  failure sets `credentialSuspected: false`, does not touch the alert issue
  (the credential just proved itself healthy), but still fails the job
  visibly. New test: `run-canary.test.mjs` → "a cleanup-step failure after a
  successful issue filing does NOT file a credential alert".
- **Medium** (`gpt-5.4`): `alert-lib.mjs`'s dedup logic (list-then-create,
  no server-side lock) could race under overlapping runs, creating duplicate
  alerts or flip-flopping open/close state. Addressed at the workflow level
  with a `concurrency: { group: dev-ingest-lifecycle, cancel-in-progress:
false }` block rather than in-script locking — GitHub Actions queues
  overlapping triggers instead of running them in parallel, which is
  sufficient to prevent the race without adding complexity to the alert
  script itself. New test: workflow test asserts the `concurrency.group`
  exists and `cancel-in-progress` is not `true`.

Both reviews were re-run against the corrected diff (OIDC live, fail-loud
preflight, concurrency gate, credentialSuspected split). That second pass
found 2 more issues, both fixed before publishing:

- **Medium** (`claude-opus-4.6`): `AZURE_CLIENT_ID`/`AZURE_TENANT_ID`/
  `AZURE_SUBSCRIPTION_ID` (preflight's existence check) and `CRAWLER_CI_PAT`
  (deploy's `az deployment group create` call) were interpolated directly via
  `${{ secrets.X }}` inside `run:` shell blocks — GitHub's own
  security-hardening guidance flags this because shell metacharacters in a
  secret value (`"`, `` ` ``, `$`, backslash, newline) can be misinterpreted,
  from silent truncation up to command injection. Fixed by passing each
  secret through an `env:` block and referencing it as a shell variable
  (`$AZURE_CLIENT_ID`, `$GITHUB_CI_PAT`, …) instead. `azure/login@v2`'s
  `with:` block was already safe (YAML property values, not shell-interpolated
  strings) and was left as-is. Updated
  `dev-ingest-lifecycle-workflow.test.ts`'s "sources the Bicep githubCiPat
  parameter from the CRAWLER_CI_PAT repo secret" case to check the deploy
  step's `env:` mapping instead of requiring a literal `secrets.CRAWLER_CI_PAT`
  substring inside `run:`.
- **High** (`gpt-5.4`): `run-canary.mjs`'s `runCanary()` never checked that
  the returned `issueUrl`'s owner/repo matched the expected
  `alertOwner`/`alertRepo` before attempting to label/close it with
  `GITHUB_TOKEN`. If the live Function's own `GITHUB_REPOSITORY` setting were
  to drift, the canary would try to mutate an issue in an unintended repo —
  and that failure would land in the `credentialSuspected: false`
  cleanup-only branch, meaning **no alert would fire** for what is actually a
  config-integrity failure, not a benign cleanup hiccup. Fixed by adding an
  explicit case-insensitive owner/repo match check right after
  `parseIssueUrl()` succeeds, inside the same code path that already handles
  POST/parse failures, so a mismatch is now treated as
  `credentialSuspected: true` (alert filed) instead of falling through to
  cleanup-only. New test: `run-canary.test.mjs` → "an issueUrl pointing at an
  unexpected repo is treated as a credential/config failure, not cleaned up".

See the PR for the final review pass results.

While running `npm run test:guards` I found and fixed a real, unrelated bug:
`scripts/agent/review/review-policy.test.mjs`'s `activeSourceFiles()` used
`path.relative()` directly, which returns backslash-separated paths on Windows.
That never matches the forward-slash entries in its own `allowedRetirementDocs`
allowlist, so on Windows the test **falsely flagged**
`scripts/agent/docs/check-adr-consistency.ts` (which is explicitly allowlisted)
as containing a retired token. Fixed by normalizing every computed path to
POSIX separators before comparison. Confirmed passing in isolation after the
fix.

Two other `test:guards` failures remain and were **not** fixed here — both are
pre-existing, Windows-environment-specific, and already documented elsewhere,
unrelated to any file this PR touches:

- `ci-recovery/reconcile.test.mjs` — 2 tests crash with the documented
  Windows `UV_HANDLE_CLOSING` libuv assertion (see e.g.
  `docs/knowledge/handoffs/2026-07-16-ci-recovery-state.md`, `2026-07-28-ci-conflict-false-labelling.md`).
- `ci-conflict-coordinator/reconcile.test.mjs` — several tests fail with
  `ECONNRESET`/timeout, consistent with the same class of known
  network/Windows flakiness in that suite, unrelated to `dev-build-ingest` or
  `dev-ingest-canary`.

Neither suite is touched by this PR's diff; both are called out here so the
next agent doesn't re-diagnose them from scratch.

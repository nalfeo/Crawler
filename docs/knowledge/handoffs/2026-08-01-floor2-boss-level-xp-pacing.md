...
- **Timing bound on *first* encounter only**: `Math.min(encounterStartedMs)` across all started families, not per-family, to avoid false failures on runs that complete all 4 dens (Win B path).

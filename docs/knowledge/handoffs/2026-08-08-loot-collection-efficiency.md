...
3. **Trivial pickup snap** (`TRAVEL_LOOT_SNAP_FT = 5`) in `pickSafeTravelHeading`:
   the corridor loot bias only *curves* the arc, so the runner slid past gems without
...
- **Widening a loot knob reliably makes collection worse, not better.** Three separate
  ...
  longer errands: mean level went *up* while wins went *down*. Narrow windows won every
  time.
...
- Ordered the deferral check before the stair-proximity check, which silently made the
  ...
  code path where the guarded action *cannot* happen is always wrong — charge the budget

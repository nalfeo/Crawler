...
maxHp`, so a base-CON(1) character starts at exactly 100 HP
   (`BASE_MAX_HP_FLOOR = 90` chosen so `90 + 10×1 === 100`) and `statSystem`'s
...
11. **Default AI allocator branches only on offense stat, sharing everything
    else.** New sequence: offense (Strength for a physical weapon,
    Intelligence for `WeaponType.MAGIC`) → 5, Constitution → 6, Dexterity → 5,

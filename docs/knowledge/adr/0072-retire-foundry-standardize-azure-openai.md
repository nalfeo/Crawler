...
🍎 x 2 — tooling-only cleanup; no runtime gameplay changes.
...
1. **`asset-request.yml`** — Drain step switches from `FOUNDRY_*` secrets and
   `SPRITES_*_PROVIDER: foundry` to `AZURE_OPENAI_*` secrets. The `azure-openai`
   backend is the default, so no `SPRITES_*_PROVIDER` override is needed.
...
5. **Tests** — Remove foundry backend test suites from `factory.test.ts`; update
...
- Removes ~120 lines of dead code and a parallel code path that was masquerading as
  something it wasn't.
- Eliminates the `FOUNDRY_BRIEF_SELECTOR_MODEL ≠ FOUNDRY_TEXT_MODEL` footgun.
- The `SPRITES_*_PROVIDER` environment variables no longer need to be set in CI
  (the `azure-openai` default is correct).

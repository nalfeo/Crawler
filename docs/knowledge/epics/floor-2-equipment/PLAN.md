...
"Aggregate DPS" is the stat-formula-derived DPS for the representative weapon at
each checkpoint: `(baseDamage + damageBonus) × (1 + damagePercent) × attackSpeedFactor × critFactor`,
where `critFactor = 1 + critChance × (critMultiplier − 1)`.
...
**Status:** `validated` once this PR merges.
...
3. Computes aggregate DPS for each checkpoint via the formula above.
4. Emits a JSON result with `ratio_1_to_6` and `ratio_6_to_11`.

Acceptance: script exits 0 and outputs both ratios to stdout as JSON.
...
Define new `EquipmentItemDef` entries in `src/shared/equipmentDefs.ts` (or a
new `equipmentDefs.floor2.ts`) for Floor 2 levels 1–5. Target slots: at least
one weapon slot (mainHand), one armor slot (torso or head), and one accessory
slot (ring or neck). Stat bonuses must be tuned to contribute a share of the
1.7–2.3× DPS uplift at the level-6 checkpoint (measured by B1 tooling).
...
Define new `EquipmentItemDef` entries for Floor 2 levels 6–11. Minimum coverage:
one weapon upgrade (higher tier than B2), one armor upgrade, one accessory upgrade.
Stat bonuses tuned to contribute the 1.7–2.3× DPS uplift at the level-11
checkpoint over the Tier-1 equipped level-6 baseline.
...
Wire Tier-2 items into mid/late Floor 2 archetypes and loot tables. Ensure at
least one Tier-1 item is purchasable by level 4 and at least one Tier-2 item is
purchasable by level 8 in a representative run.

Acceptance: `generateShopInventory` returns the new items for the appropriate
archetypes; headless run logs show item purchases at the correct level windows.

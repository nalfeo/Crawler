...
1. The merge-train reconciler scans open same-repository PRs oldest-first and
   adds `merge-train` after the PR head's admission checks pass and all review
   threads resolve. A substantive Copilot review is not required for admission.
   Local Ducky or Codex review may be recorded in a PR description, commit
   message, or PR comment by stating that review passed and the change is okay
   to check in; that record is evidence for humans, not a machine-parsed gate.
